...
public class PasswordResetTokenIntegrationTest extends AbstractIntegrationTest {
    ...
    public void testDeleteToken() throws Exception {
        ...
        PasswordResetToken shouldNotExistToken = (PasswordResetToken) passwordResetTokenRepository.findOne(tokenId);
    ...
    public void testMultipleTokensAreReturnedWhenQueringByUserId() throws Exception {
        ...
        Collections.sort(tokensAsList);

        for(int i=0;i<tokensAsList.size();i++){
            System.out.println(tokensAsList.get(i));
        }


        List<String> actualTokensAsList = actualTokens.stream().map(prt -> prt.getToken()).collect(Collectors.toList());
        Collections.sort(actualTokensAsList);
        for(int i=0;i<actualTokensAsList.size();i++){
            System.out.println(actualTokensAsList.get(i));
        }
